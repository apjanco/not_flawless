"""
evaluators/paddleocr_eval.py
PaddleOCR model evaluator
"""
...
def evaluate(project_root: str = None) -> Dict[str, Any]:
    ...
    if not check_dependencies():
        log_error(MODEL_NAME, "PaddleOCR not installed")
        return {"error": "PaddleOCR not available"}
    
    # Initialize model
    try:
        log_info(MODEL_NAME, "Initializing PaddleOCR model")
    ...
    # Run evaluation
    metrics = _run_evaluation(ocr, test_images, test_labels)
...
def _run_evaluation(ocr, image_paths: List[str], ground_truths: List[str]) -> Dict[str, Any]:
    """

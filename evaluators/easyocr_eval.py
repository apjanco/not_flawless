"""
evaluators/easyocr_eval.py
EasyOCR model evaluator
"""
...
def evaluate(project_root: str = None) -> Dict[str, Any]:
    ...
    if not check_dependencies():
        log_error(MODEL_NAME, "EasyOCR not installed")
        return {"error": "EasyOCR not available"}
    
    # Initialize model
    try:
        log_info(MODEL_NAME, "Initializing EasyOCR model for English")
    ...
    # Run evaluation
    metrics = _run_evaluation(reader, test_images, test_labels)
...
def _run_evaluation(reader, image_paths: List[str], ground_truths: List[str]) -> Dict[str, Any]:
    """

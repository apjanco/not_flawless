"""
evaluators/pylaia_eval.py
PyLaia handwriting text recognition model evaluator
"""
...
try:
    from laia.common import LAIA_ROOT
    from laia.models import htr_crnn
except ImportError:
    LAIA_ROOT = None
    htr_crnn = None
...
def check_dependencies():
    """Check if PyLaia is installed"""
    return LAIA_ROOT is not None and htr_crnn is not None
...
def evaluate(project_root: str = None) -> Dict[str, Any]:
    ...
    if not check_dependencies():
        log_error(MODEL_NAME, "PyLaia not installed or not properly configured")
        return {"error": "PyLaia not available"}
    ...
    # Initialize model
    try:
        log_info(MODEL_NAME, "Initializing PyLaia model")
        # PyLaia model initialization would go here
        # This is a placeholder pending specific model configuration
...
def _run_evaluation(image_paths: List[str], ground_truths: List[str]) -> Dict[str, Any]:
    ...
    for idx, (image_path, ground_truth) in enumerate(zip(image_paths, ground_truths)):
        ...
        try:
            ...
            # PyLaia inference would go here
